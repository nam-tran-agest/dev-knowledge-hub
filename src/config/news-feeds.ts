import { Globe, Cpu, Gamepad, Zap, Tv, Languages, HeartPulse, Briefcase, Trophy, GraduationCap } from 'lucide-react';
import React from 'react';

export const FEEDS = [
    // --- General Feeds ---
    {
        url: 'https://tuoitre.vn/rss/tin-moi-nhat.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png'
    },
    {
        url: 'https://dantri.com.vn/rss/home.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png'
    },
    {
        url: 'https://thanhnien.vn/rss/home.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg'
    },
    {
        url: 'https://vnexpress.net/rss/tin-moi-nhat.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg'
    },
    {
        url: 'https://plo.vn/rss/home.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png'
    },

    // --- News Category Feeds ---
    {
        url: 'https://tuoitre.vn/rss/thoi-su.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'social'
    },
    {
        url: 'https://thanhnien.vn/rss/thoi-su.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'social'
    },
    {
        url: 'https://dantri.com.vn/rss/thoi-su.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'social'
    },
    {
        url: 'https://vnexpress.net/rss/thoi-su.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'social'
    },
    {
        url: 'https://plo.vn/rss/thoi-su-1.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'social'
    },

    // --- World Category Feeds ---
    {
        url: 'https://tuoitre.vn/rss/the-gioi.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'world'
    },
    {
        url: 'https://thanhnien.vn/rss/the-gioi.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'world'
    },
    {
        url: 'https://vnexpress.net/rss/the-gioi.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'world'
    },
    {
        url: 'https://dantri.com.vn/rss/the-gioi.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'world'
    },
    {
        url: 'https://plo.vn/rss/quoc-te-8.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'world'
    },

    // --- Sports Category Feeds ---
    {
        url: 'https://tuoitre.vn/rss/the-thao.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'sports'
    },
    {
        url: 'https://thanhnien.vn/rss/the-thao.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'sports'
    },
    {
        url: 'https://vnexpress.net/rss/the-thao.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'sports'
    },
    {
        url: 'https://dantri.com.vn/rss/the-thao.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'sports'
    },
    {
        url: 'https://plo.vn/rss/the-thao-22.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'sports'
    },

    // --- Health Category Feeds ---
    {
        url: 'https://tuoitre.vn/rss/suc-khoe.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'health'
    },
    {
        url: 'https://thanhnien.vn/rss/suc-khoe.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'health'
    },
    {
        url: 'https://vnexpress.net/rss/suc-khoe.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'health'
    },
    {
        url: 'https://dantri.com.vn/rss/suc-khoe.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'health'
    },
    {
        url: 'https://plo.vn/rss/suc-khoe-17.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'health'
    },

    // --- Entertainment Category Feeds ---
    {
        url: 'https://tuoitre.vn/rss/giai-tri.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'entertainment'
    },
    {
        url: 'https://vnexpress.net/rss/giai-tri.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'entertainment'
    },
    {
        url: 'https://thanhnien.vn/rss/giai-tri.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'entertainment'
    },
    {
        url: 'https://dantri.com.vn/rss/giai-tri.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'entertainment'
    },
    {
        url: 'https://plo.vn/rss/van-hoa-16.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'entertainment'
    },

    // --- Business Category Feeds ---
    {
        url: 'https://tuoitre.vn/rss/kinh-doanh.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'business'
    },
    {
        url: 'https://vnexpress.net/rss/kinh-doanh.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'business'
    },
    {
        url: 'https://thanhnien.vn/rss/kinh-te.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'business'
    },
    {
        url: 'https://dantri.com.vn/rss/kinh-doanh.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'business'
    },
    {
        url: 'https://plo.vn/rss/kinh-te-13.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'business'
    },

    // --- Education Category Feeds ---
    {
        url: 'https://tuoitre.vn/rss/giao-duc.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'education'
    },
    {
        url: 'https://vnexpress.net/rss/giao-duc.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'education'
    },
    {
        url: 'https://thanhnien.vn/rss/giao-duc.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'education'
    },
    {
        url: 'https://dantri.com.vn/rss/giao-duc.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'education'
    },
    {
        url: 'https://plo.vn/rss/giao-duc-21.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'education'
    },

    // --- Tech & Science Feeds ---
    {
        url: 'https://tuoitre.vn/rss/nhip-song-so.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'tech-science'
    },
    {
        url: 'https://tuoitre.vn/rss/khoa-hoc.rss',
        author: 'Tuổi Trẻ',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/archive/1/1f/20210912015238%21Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg/120px-Tu%E1%BB%95i_Tr%E1%BA%BB_Logo.svg.png',
        category: 'tech-science'
    },
    {
        url: 'https://thanhnien.vn/rss/cong-nghe.rss',
        author: 'Thanh Niên',
        logo: 'https://static.thanhnien.com.vn/thanhnien.vn/image/logo-40-nam-trang-chu.svg',
        category: 'tech-science'
    },
    {
        url: 'https://vnexpress.net/rss/khoa-hoc-cong-nghe.rss',
        author: 'VNExpress',
        logo: 'https://s1.vnecdn.net/vnexpress/restruct/i/v9779/v2_2019/pc/graphics/logo.svg',
        category: 'tech-science'
    },
    {
        url: 'https://dantri.com.vn/rss/cong-nghe.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'tech-science'
    },
    {
        url: 'https://dantri.com.vn/rss/khoa-hoc.rss',
        author: 'Dân Trí',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Dan-tri.png',
        category: 'tech-science'
    },
    {
        url: 'https://PLO.vn/rss/ky-nguyen-so-26.rss',
        author: 'PLO',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png/1280px-Logo_B%C3%A1o_Ph%C3%A1p_Lu%E1%BA%ADt_TP.HCM.png',
        category: 'tech-science'
    },

    // --- Gaming Feeds ---
    {
        url: 'https://www.pcgamer.com/rss/',
        author: 'PC Gamer',
        logo: 'https://www.pcgamer.com/apple-touch-icon.png',
        category: 'gaming'
    },
    {
        url: 'https://blog.playstation.com/feed/',
        author: 'PlayStation Blog',
        logo: 'https://blog.playstation.com/favicon-192x192.png',
        category: 'gaming'
    },
    {
        url: 'https://www.gamespot.com/feeds/mashup',
        author: 'GameSpot',
        logo: 'https://www.gamespot.com/apple-touch-icon.png',
        category: 'gaming'
    },
    {
        url: 'https://www.vg247.com/feed',
        author: 'VG247',
        logo: 'https://www.vg247.com/apple-touch-icon.png',
        category: 'gaming'
    },
    {
        url: 'https://wolfsgamingblog.com/feed/',
        author: "Wolf's Gaming Blog",
        logo: 'https://wolfsgamingblog.com/wp-content/uploads/2023/03/cropped-wolfsgamingblog-logo.jpg?w=192',
        category: 'gaming'
    },
];

export const CATEGORIES = [
    { id: 'all', name: 'All', icon: 'Globe', active: true },
    { id: 'social', name: 'Social', icon: 'Zap' },
    { id: 'world', name: 'World', icon: 'Languages' },
    { id: 'sports', name: 'Sports', icon: 'Trophy' },
    { id: 'tech-science', name: 'Technology & Science', icon: 'Cpu' },
    { id: 'gaming', name: 'Gaming', icon: 'Gamepad' },
    { id: 'health', name: 'Health', icon: 'HeartPulse' },
    { id: 'entertainment', name: 'Entertainment', icon: 'Tv' },
    { id: 'business', name: 'Business', icon: 'Briefcase' },
    { id: 'education', name: 'Education', icon: 'GraduationCap' },
];
